...
0x0C_ Задать номер ноты для канала
      ...
      номер_входа 
      нота 
0x0D_ Задать темп метронома в ударах в минуту и громкость
      0x0D
      темп
      громкость
...
void set_note_0D(byte * array, unsigned array_size) { // 0x0D_ Задать темп метронома в ударах в минуту и громкость
  if ( array[4] == 0 ) {
    cfg.metronom = 0;
    return;
  }
